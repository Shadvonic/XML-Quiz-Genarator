...
function addQuestion() {
  ...
  if ((passingQuestions === "" || passingQuestions === "0") && (score === "" || score === "0")) {
    alert("Please enter data in either the 'Number of Questions Needed to Pass' field or the 'Score' field.");
    return;
  }

  if (passingQuestions !== "" && !/^\d+$/.test(passingQuestions)) {
...
// Function to update the number of passing questions based on the passing score
...
function updateScore() {
  ...
  if (passingQuestionsInput.value.trim() !== "") {
    scoreInput.value = 0;
  }
}
...
function previewXmlData() {
  ...
  const passingQuestionsInput = document.getElementById("passingQuestions");
  const passingQuestions = passingQuestionsInput.value.trim();
...
function convertToXML(data, passingQuestions, score) {
  ...
  if (passingQuestions !== 0) {
    xmlString += ` score="${passingQuestions}"`;
  } else if (score !== 0) {
...
scoreInput.addEventListener("input", updatePassingQuestions);
passingQuestionsInput.addEventListener("input", updateScore);
