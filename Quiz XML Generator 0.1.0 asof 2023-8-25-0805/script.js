...
function updatePassingQuestions() {
  ...
  if (scoreInput.value.trim() !== 0) {
    const score = parseInt(scoreInput.value);
    const validScore = Math.min(100, Math.max(0, score)); // Ensure score is between 0 and 100
    const totalQuestions = numQuestions; // Use the global variable numQuestions here
    const passingQuestions = Math.ceil(totalQuestions * (validScore / 100));
    passingQuestionsInput.value = passingQuestions;
    scoreInput.value = validScore; // Update input value with the valid score
  } else {
