...
function addQuestion() {
  ...
  if (isNaN(numChoices) || numChoices < 2 || numChoices > 4|| !/^[0-9]+$/.test(numChoices)) {
    alert("Please enter a valid number of choices between 2 and 4.");
    return;
  }
...
function updateScore() {
  ...
  if (!isNaN(passingQuestionsValue) && passingQuestionsValue < 0) {
    passingQuestionsInput.value = 0;
  }

  const validPassingQuestions = Math.min(totalQuestions, Math.max(passingQuestionsValue, 0));
