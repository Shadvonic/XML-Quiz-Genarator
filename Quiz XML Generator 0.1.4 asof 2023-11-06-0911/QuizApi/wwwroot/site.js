...
function saveQuizToSql() {
    const passingQuestions = parseInt(document.getElementById("passingQuestions").value.trim());
    const score = parseInt(document.getElementById("score").value.trim());
    const quizData = collectQuizData();
    const xmlData = convertToXML(quizData, passingQuestions, score);

    fetch('https://localhost:7166/api/Quiz', {
        method: 'POST',
        headers: {
